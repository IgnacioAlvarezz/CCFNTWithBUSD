...
echo "Trader ($TRADER_ADDRESS) approving CCNFT ($CCNFT_CONTRACT_ADDRESS) to spend BUSD tokens..."
# Approves a very large amount to cover multiple transactions
cast send "$BUSD_CONTRACT_ADDRESS" "approve(address,uint256)" "$CCNFT_CONTRACT_ADDRESS" 1000000000000000000000000000 $CAST_COMMON "$TRADER_PRIVATE_KEY"
cast send "$BUSD_CONTRACT_ADDRESS" "approve(address,uint256)" "$TRADER_ADDRESS" 1000000000000000000000000000 $CAST_COMMON "$TRADER_PRIVATE_KEY"
